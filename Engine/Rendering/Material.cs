       ...
       public void Bind()
       {
          ...
          for (int i = 0; i < this.Textures.Count; i++)
          {
             ...
             catch (Exception e)
             {
                if (Debug.Config.logShaderExceptions)
                {
                   ...
                   Debug.LogException(new Exception("Failed to set texture: '" + names[i] + "' - Error: " + e.Message));
                }
             }
          }
          ...
          foreach (KeyValuePair<string, Tuple<object, Type>> uniformData in this.Uniforms)
          {
             ...
             catch (Exception e)
             {
                ...
                {
                   // Log error but continue rendering (graceful degradation)
                   Debug.LogException(new Exception("Failed to set uniform: '" + uniformData.Key + "' - Error: " + e.Message));
                }
             }
          }
       }
